Feature: Perform different actions connected with search requests on SearchPage
  ...
  @MapMarks
  Scenario Outline: check if map mark has information that fit search request criteria
  ...
  @SearchByConditions
  Scenario Outline: check if every result fit search request criteria
    Given user perform search by "<CITY_NAME>"
    When user create search request by "<MIN_PRICE_VALUE>", "<MAX_PRICE_VALUE>","<BED_NUMBER>", "<BATH_NUMBER>","<MIN_SQFT_VALUE>", "<MAX_SQFT_VALUE>"
  ...
  @FoundHouses
  Scenario: check if number of found elements match result information
    Given user perform search by "Berkeley, CA"
    Then number of found houses match result information

  @SortByPrice
  Scenario Outline: check if elements on the page are displayed sorted by option
    Given user perform search by "<CITY_NAME>"
  ...
  @SchoolRating
  Scenario Outline: check if schools that are displayed on map have rating more than selected
    Given user perform search by "<CITY_NAME>"
  ...
  @RestaurantsFilter
  Scenario: check if all marks on the map are restaurants
